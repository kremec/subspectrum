...
class CPDTest {
    ...
    fun decodeInstruction() {
        val instruction = CPD.decode(0xEDA9L, 0x1000u)

        assertEquals(0x1000u, instruction.address)
        assertEquals(2, instruction.bytes.size)
        assertEquals(0xED.toByte(), instruction.bytes[0])
        assertEquals(0xA9.toByte(), instruction.bytes[1])
    }

    @Test
    fun executeCompareAndDecrement() {
        ...
        val instruction = CPD(
            address = 0x1000u,
            bytes = DataByteArray(byteArrayOf(0xED.toByte(), 0xA9.toByte()))
        )
    ...
    fun toStringFormat() {
        ...
        assertEquals("CPD", instruction.toString())
    }
}
