...
class LDIRTest {
    ...
    fun decodeInstruction() {
        val instruction = LDIR.decode(0xEDB0L, 0x1000u)

        assertEquals(0x1000u, instruction.address)
        assertEquals(2, instruction.bytes.size)
        assertEquals(0xED.toByte(), instruction.bytes[0])
        assertEquals(0xB0.toByte(), instruction.bytes[1])
    }

    @Test
    fun executeLoadIncrementRepeat() {
        ...
        val instruction = LDIR(
            address = 0x1000u,
            bytes = DataByteArray(byteArrayOf(0xED.toByte(), 0xB0.toByte()))
        )
