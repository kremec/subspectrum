...
class EXSPIYTest {
    ...
    fun decodeInstruction() {
        val instruction = EXSPIY.decode(0xFDE3L, 0x1000u)
    ...
    @Test
    fun executeSwapIYWithSP() {
        Registers.specialPurposeRegisters.setIY(0x1234.toShort())
        ...
        val instruction = EXSPIY(
            address = 0x1000u,
            bytes = DataByteArray(byteArrayOf(0xFD.toByte(), 0xE3.toByte()))
        )

        instruction.execute()

        assertEquals(0xCDAB.toShort(), Registers.specialPurposeRegisters.getIY())
    ...
    fun toStringFormat() {
        ...
        assertEquals("EX (SP), IY", instruction.toString())
    }
}
