...
class OTIRTest {
    ...
    fun decodeInstruction() {
        val instruction = OTIR.decode(0xEDB3.toLong(), 0x1000u)

        assertEquals(0x1000u, instruction.address)
        assertEquals(2, instruction.bytes.size)
        assertEquals(0xED.toByte(), instruction.bytes[0])
        assertEquals(0xB3.toByte(), instruction.bytes[1])
    }

    @Test
    fun executeOutputIncrementRepeatBNotZero() {
        ...
        val instruction = OTIR(
            address = 0x1000u,
            bytes = DataByteArray(byteArrayOf(0xED.toByte(), 0xB3.toByte()))
        )
    ...
    @Test
    fun executeOutputIncrementRepeatBZero() {
        // Set up PC at instruction
        Registers.specialPurposeRegisters.setPC(0x1000.toShort())
    ...
    fun toStringFormat() {
        ...
        assertEquals("OTIR", instruction.toString())
    }
}
