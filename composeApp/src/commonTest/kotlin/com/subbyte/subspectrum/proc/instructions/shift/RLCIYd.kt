...
class RLCIYdTest {
    ...
    fun decodeInstruction() {
        val instruction = RLCIYd.decode(0xFDCB0006L, 0x1000u)
        ...
        val rlcIYd = instruction as RLCIYd
        assertEquals(0x00.toByte(), rlcIYd.displacement)
    }

    @Test
    fun executeRotateLeftMemoryIYd() {
        Registers.specialPurposeRegisters.setIY(0x2000.toShort())
        Memory.memorySet.setMemoryCell(0x2000u, 0x80.toByte()) // 10000000

        val instruction = RLCIYd(
            address = 0x1000u,
            bytes = DataByteArray(byteArrayOf(0xFD.toByte(), 0xCB.toByte(), 0x00.toByte(), 0x06.toByte())),
    ...
    fun toStringFormat() {
        ...
        assertEquals("RLC (IY+00h)", instruction.toString())
    }
}
