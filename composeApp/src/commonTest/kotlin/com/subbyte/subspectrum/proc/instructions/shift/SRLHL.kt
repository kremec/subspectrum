...
class SRLHLTest {
    ...
    fun decodeInstruction() {
        val instruction = SRLHL.decode(0xCB3EL, 0x1000u)

        assertEquals(0x1000u, instruction.address)
        assertEquals(2, instruction.bytes.size)
        assertEquals(0xCB.toByte(), instruction.bytes[0])
        assertEquals(0x3E.toByte(), instruction.bytes[1])
    }

    @Test
    fun executeLogicalShiftRightMemoryHLWithCarry() {
        Registers.registerSet.setHL(0x2000.toShort())
        Memory.memorySet.setMemoryCell(0x2000u, 0x03.toByte()) // 00000011

        val instruction = SRLHL(
            address = 0x1000u,
            bytes = DataByteArray(byteArrayOf(0xCB.toByte(), 0x3E.toByte()))
        )
    ...
    @Test
    fun executeLogicalShiftRightMemoryHLWithCarryFalse() {
        Registers.registerSet.setHL(0x2000.toShort())
    ...
    @Test
    fun executeLogicalShiftRightMemoryHLNegativeToPositive() {
        Registers.registerSet.setHL(0x2000.toShort())
        ...
        ) // 01000010 (shifted right, MSB set to 0, becomes positive)
    ...
    fun toStringFormat() {
        ...
        assertEquals("SRL (HL)", instruction.toString())
    }
}
